...
//Which ultimately are retrieved and reacted to by GameEngine2D/StarterGame classes
...
public class Window extends JComponent implements KeyListener, MouseListener{
    ...
    //Linked to the Collection of the associated GameEngine2D object
    private DisplayList entities;    
    ...
    public Window(int windowWidth, int windowHeight, DisplayList entities){
        this.entities = entities; //Link this entities to the respective GameEngine2D
    ...
    private void initLogic(){
        ...
        //Create a set of all the movement keys, as defined in GameEngine2D
        for (int key : GameEngine2D.MOVEMENT_KEYS)
    ...
    //Called by GameEngine2D to redraw the window and all of the Entities in it   
    public void refresh(){
        this.repaint();
    ...
    private void drawEntities(Graphics g){
        for (int i = 0; i < entities.size(); i++){
            ...
            try{
                Image img = loadedImages.get(e.getImage());
    ...
    public void updateDebugText(){
        StringBuilder debugText = new StringBuilder(DEBUG_PREFIX + "DisplayList Size: " + entities.size());
    ...
    public void keyPressed(KeyEvent event) { 
        ...
        if (keyCode == GameEngine2D.KEY_QUIT_GAME)
            System.exit(0);    
        else if (keyCode == GameEngine2D.KEY_TOGGLE_DEBUG)
